...
async function main() {
  ...
  if (existing.has('battery_life_estimate')) {
    console.log('device_specs.battery_life_estimate: already exists');
    return;
  }

  if (!APPLY) {
    console.log('DRY RUN add device_specs.battery_life_estimate (SingleLineText)');
    return;
  }
  ...
  await apiFetch(`${baseUrl}/api/v2/meta/tables/${specsTable.id}/columns`, token, {
    method: 'POST',
    body: JSON.stringify({
      title: 'battery_life_estimate',
      column_name: 'battery_life_estimate',
      uidt: 'SingleLineText',
      dt: 'varchar',
      pk: false,
      rqd: false,
  ...
  console.log('Added device_specs.battery_life_estimate (SingleLineText)');
}
